...
const DEFAULT_PROFILE_IMAGE = '../../assets/images/profile-1.png';
    ...
    if (authResult.success) {
      ...
      setTimeout(() => {
        ...
        if (profileBox) {
          ...
          if (profileImage) {
            profileImage.src = user.profileImage || DEFAULT_PROFILE_IMAGE;
          }
        }
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    function checkForNewSupportMessages() {
      ...
      if (newMessageNotification && parseInt(newMessageNotification) > parseInt(lastCheck)) {
        ...
        // Não recarregar o chat se já está aberto (o listener em tempo real cuida disso)

        // Apenas atualizar se não houver chat aberto

        if (!currentSupportChatId) {

          // Sem chat aberto, apenas atualizar lista
    ...
    function createContactSelectorItem(id, name, image, type, contactType) {
      ...
      if (image && contactType === 'normal') {
        ...
        avatarHTML = `<img src="${image}" onerror="this.src='https://via.placeholder.com/36x36?text=${safeInitial}'" alt="${safeName}">`;
    ...
    window.redirectRecruitmentRequest = function(requestId) {
      ...
      const redirectUrl = '../vagas-publicas.html';
